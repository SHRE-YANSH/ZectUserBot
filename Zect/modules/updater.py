...
async def upstream(client, message):
    ...
    try:
        ...
        if conf != "now":
            await status.edit(f"**Unfortunately, the directory {error} does not seem to be a git repository.Or Maybe it just needs a sync verification with {GIT_REPO_NAME} But we can fix that by force updating the userbot using** `.update now.`")
            return
        repo = Repo.init()
    ...
    if "now" not in conf:
        if changelog:
            ...
            if len(changelog_str) > 4096:
                ...
                return await status.edit("{changelog_str}\n\nDo `.update now` to update."
                )
        else:
    ...
    if HEROKU_API is not None:
        ...
        if heroku_app is None:
            ...
            repo.__del__()
            return
        await status.edit("`Userbot dyno build in progress, please wait for it to complete.`")
        ups_rem.fetch(ac_br)
